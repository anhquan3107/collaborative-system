...
import { createProject, getProjects, deleteProject, getProjectCount } from "../../../api/project.js";
...
export async function loadProjects() {
    try {
        ...
        if (data.projects.length === 0) {
            projectList.innerHTML = "<div class='col-12'><p class='text-center text-muted'>No projects yet.</p></div>";
            return;
        }
                ...
                <div class="col-xl-20 col-lg-3 col-md-4 col-sm-6 mb-4">
                    <div class="card shadow-sm h-100 project-card">
                        <div class="card-body d-flex flex-column">
                            <div style="cursor: pointer;" class="mb-2 flex-grow-1" onclick="openProject(${project.id}, '${project.name.replace(/'/g, "\\'")}')">
                                <h5 class="card-title mb-1 text-primary font-weight-bold text-truncate" title="${project.name}">${project.name}</h5>
                                <p class="card-text small text-muted text-truncate">${project.description || "No description"}</p>
                            </div>
                            
                            <div class="mt-auto d-flex justify-content-between align-items-center">
                                <small class="text-gray-500"><i class="fas fa-users"></i> Members</small>
                                <button class="btn btn-outline-danger btn-sm py-0" style="font-size: 0.8rem;" onclick="deleteProjectHandler(${project.id})">
        ...
        projectList.innerHTML = "<div class='col-12'><p class='text-danger text-center'>No projects found</p></div>";
    }
}
